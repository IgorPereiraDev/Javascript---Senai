...
$siglaDigitado = document.getElementById("txtdigite");
$botaoPesquise = document.getElementById("btnpesquisar");
$mostraDadosHtml = document.getElementById("div_mostra_dados");
...
//Serve para filtrar o estado e a cidade
const funcFiltros = (  ) => {

   estadoDigitado = $siglaDigitado.value.toUpperCase();

   //Essa função esta recebendo o json cidade, e pegando a sigla do estado
   const filtrarEstado = ( cidade ) => cidade.sigla == estadoDigitado;

   //Essa variavel esta filtrando o json cidade e recebendo o filtrar para pegar o array da sigla do filtrarEstado
   const filtrarCidades = cidade.filter( filtrarEstado );

   //Essa variavel esta pegando o tamanho do indice cidades
   const tamanho = filtrarCidades[0].cidades.length;

   envia = forCidades(tamanho, filtrarCidades);
}

//Essa função manda os dados para o html
const mostrarDados = ( recebeCidade, recebeQuantidade ) => {

   console.log(cor);
   if (cor == true)
   {
      
      $mostraDadosHtml.innerHTML += `
      <div class="caixa_resultado cor1">
         <div class="div_quantidade">
            ${recebeQuantidade}
         </div>

         <div class="div_cidades">
            ${recebeCidade}
         </div>
      </div>`;
      cor = false;
      ...
      $mostraDadosHtml.innerHTML += `
      <div class="caixa_resultado cor2">
         <div class="div_quantidade">
            ${recebeQuantidade}
         </div>
...
//Serve para sobrescrever as cidades e não re repetir abaixo dela
const verificaVazio = (  ) => {
   if(!$mostraDadosHtml == "")
   {
      $mostraDadosHtml.innerHTML = "";
...
//Essa estrutura de repetição serve para mostrar todas as cidades do estado
const forCidades = ( tamanho, filtrarCidades ) => {
   for (let i = 0; i < tamanho; i++) {
      let recebeCidade = filtrarCidades[0].cidades[i].nome;

      let f = i + 1;

      var mostrarCidades = recebeCidade;
      var mostrarQuantidade = f;

      Envia = mostrarDados(mostrarCidades, mostrarQuantidade);

   }
}

//Serve para funcionar quando clicar o botão
$botaoPesquise.addEventListener("click",()=>verificaVazio());
$botaoPesquise.addEventListener("click",()=>funcFiltros());
// $botaoPesquise.addEventListener("click",()=>confereVazio($mostraDadosHtml));